...
export const initialState = {
  ...
  gameStatus: "Make your next move",
...
const fillBoardValues = (tileBoard, initialPositionX, initialPositionY) => {
  ...
  while (bombsCreated !== bombsCount) {
    ...
    if (bombTile.value === -1 || positionX === initialPositionX || positionY === initialPositionY) {
      continue
    }
    ...
    let isAroundInitial = tilesAround.some(tile => {
      return tile.positionX === initialPositionX && tile.positionY === initialPositionY
    })
...
const flipZerosInArea = (tileBoard, positionX, positionY) => {
  getTilesAround(tileBoard, positionX, positionY)
    ...
    .map(tileAround => flipZerosInArea(tileBoard, tileAround.positionX, tileAround.positionY))
}

const flipAllBombs = (tileBoard) => {
  tileBoard.forEach(tileLine => {
    tileLine.filter(tile => tile.value === -1).forEach(tile => {
      tile.flipped = true
    })
  })
}
...
const flagTile = (state, selectedTile, flag) => {
  ...
  if (selectedTile.flag === flag)
    selectedTile.flag = null
  else
    selectedTile.flag = flag
}
...
export const gameSlice = createSlice({
  ...
  reducers: {
    resetGame: state => {
      ...
      state.gameStatus = "Make your next move"
      state.moves = 0
      state.currentTool = TOOLS.SHOVEL
    },
    makeMove: (state, { payload }) => {
      if (state.gameStatus !== "Make your next move") return
...
export const { makeMove, resetGame, toggleShovel, toggleFlagBomb, toggleFlagUnknown } = gameSlice.actions

export const gameSelector = (positionX, positionY) => state => state.game.tileBoard[positionX][positionY]
